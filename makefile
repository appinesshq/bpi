...
kind-load:
	...
	kind load docker-image metrics-amd64:1.0 --name bpi-cluster
...
kind-api: api
	kind load docker-image bpi-api-amd64:1.0 --name bpi-cluster
	kubectl delete pods -lapp=bpi-api
...
kind-logs:
	kubectl logs -lapp=bpi-api --all-containers=true -f
...
kind-status-full:
	kubectl describe pod -lapp=bpi-api

kind-shell:
	kubectl exec -it $(shell kubectl get pods | grep bpi-api | cut -c1-26) --container app -- /bin/sh
...
migrate:
	go run app/bpi-admin/main.go migrate
...
gcp-upload:
	docker tag bpi-api-amd64:1.0 gcr.io/$(PROJECT)/bpi-api-amd64:1.0
	docker tag metrics-amd64:1.0 gcr.io/$(PROJECT)/metrics-amd64:1.0
	docker push gcr.io/$(PROJECT)/bpi-api-amd64:1.0
...
gcp-services:
	...
	# kubectl create -f deploy-bpi-api.yaml
	# kubectl expose -f expose-bpi-api.yaml --type=LoadBalancer

gcp-status:
	gcloud container clusters list
	kubectl get nodes
	kubectl get pods
	kubectl get services bpi-api

gcp-logs:
	kubectl logs -lapp=bpi-api --all-containers=true -f

gcp-shell:
	kubectl exec -it $(shell kubectl get pods | grep bpi-api | cut -c1-26 | head -1) --container app -- /bin/sh
	# ./admin --db-disable-tls=1 migrate
	# ./admin --db-disable-tls=1 seed

gcp-delete:
	kubectl delete services bpi-api
	kubectl delete deployment bpi-api	
	gcloud container clusters delete $(CLUSTER)
	gcloud projects delete bpi-api
	gcloud container images delete gcr.io/$(PROJECT)/bpi-api-amd64:1.0 --force-delete-tags
	gcloud container images delete gcr.io/$(PROJECT)/metrics-amd64:1.0 --force-delete-tags
	docker image remove gcr.io/bpi-api/bpi-api-amd64:1.0
	docker image remove gcr.io/bpi-api/metrics-amd64:1.0
